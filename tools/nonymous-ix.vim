...
if s:env =~ 'LINUX' && executable('xclip')
  command! -range=% IX <line1>,<line2>w !curl -n -s -X POST --data-binary @- https://no.dungeon.red | tr -d '\n' | tee >(xclip -i -selection clipboard)
  command! -nargs=1 -range=% RX <line1>,<line2>w !curl -n -s -X PUT --data-binary @- <args> | tr -d '\n' | tee >(xclip -i -selection clipboard)
